...
int main(int argc, char** argv)
{
    ...
    cout << "CPU compute time: " << duration << "us" << endl << endl;
    ...
    size_t bsx = 256;
    size_t gsx = (width + bsx - 1)/bsx;
    size_t nsubhist = gsx;
    dim3 subHistGridDim(gsx, 1, 1);
    ...
    CUDA_CHECK(cudaMemset(histogram, 0, sizeof(uint32_t) * nbins));
    CUDA_CHECK(cudaMemset2D(subHistogram, subHistogramPitch, 0, sizeof(uint32_t) * nbins, nsubhist));
    ...
    for(int i=0; i<100; i++)
    {
        clearSubHistograms<<<sumGridDim, sumBlockDim>>>(subHistogram, nbins, nsubhist, subHistogramPitch);
        CUDA_CHECK(cudaDeviceSynchronize());

        t1 = chrono::steady_clock::now();
        subhist<<<subHistGridDim, subHistBlockDim>>>(inputDevice, width, height, inputPitch, subHistogram, nbins, nsubhist, subHistogramPitch);
    ...
    for(size_t i=0; i<nbins; i++)
    {
        if(histogramHost[i] != (uint32_t)cvHist.at<float>(i))
        {
            ...
            cout << i << ": " << histogramHost[i] << " vs " << (uint32_t)cvHist.at<float>(i)<<endl;
        }
    }
